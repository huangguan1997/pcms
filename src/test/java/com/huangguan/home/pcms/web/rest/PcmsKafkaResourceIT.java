...
import com.huangguan.home.pcms.config.KafkaProperties;
...
class PcmsKafkaResourceIT {
    ...
    void setup() {
        ...
        PcmsKafkaResource kafkaResource = new PcmsKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/pcms-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/pcms-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
